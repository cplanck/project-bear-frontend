...
export default function Dashboard() {
  ...
  useEffect(()=>{
    let instrument = instruments.filter((item)=>item.id == instrumentId?item:'')
    instrument.length==1?setInstrument(instrument[0]):''
  })
    ...
    <div className={styles.instrumentContainer}>
      ...
      <div className={styles.mainPanel}>
        <Container maxWidth={false} sx={{ maxWidth: '1800px' }}>
          <div>
            <InstrumentHeading instrument={instrument} instrumentId={instrumentId}/>
          </div>
          <div style={{border: '0px solid blue', height: '100vh'}}>
            <div className='activePage'><InstrumentDetails instrument={instrument} setInstruments={setInstruments}/></div>
            </div>
        </Container>
      </div>
    </div>
